...
class TranscodeEngine(QObject):
    ...
    def get_base_vpy_template(self):
        ...
        default_template = (
            ...
            "    except AttributeError:\n"
            "        import os\n"
            "        dll_path = r\"{components_dir}/vapoursynth/plugins/subtext.dll\"\n"
            ...
            "        except Exception as e:\n"
            "            raise RuntimeError(f\"字幕渲染失败：加载字幕插件失败 ({dll_path})，错误信息: {e}\")\n"
            "        try:\n"
            "            clip = core.sub.TextFile(clip, file=sub_path)\n"
            "        except Exception as e:\n"
            "            raise RuntimeError(f\"字幕渲染失败：解析或载入字幕文件 '{sub_path}' 出错: {e}\")\n"
            "    except Exception as e:\n"
            "        raise RuntimeError(f\"字幕渲染失败：解析或载入字幕文件 '{sub_path}' 出错: {e}\")\n\n"
            "# 9. 输出\n"
            "clip.set_output()\n"
